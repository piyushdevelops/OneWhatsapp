...
const PORT = Number(process.env.INBOX_API_PORT || 8790);
const EVENTS_FILE = path.join(__dirname, "webhook-events.jsonl");
const REPLIES_FILE = path.join(__dirname, "local-replies.json");
...
function fromProviderTimestamp(timestamp, receivedAt) {
  ...
  // Meta test webhooks sometimes ship sample timestamps from old fixtures.
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && detailMatch) {
    ...
    if (!conversation) return send(res, 404, { error: "conversation_not_found" });
    return send(res, 200, conversationResponse(conversation));
  }
  ...
  if (req.method === "POST" && replyMatch) {
    ...
    if (!body.body || typeof body.body !== "string") {
      return send(res, 400, { error: "body_required" });
    }
    const reply = {
      id: `local_${Date.now()}`,
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`Inbox API server listening on http://127.0.0.1:${PORT}`);
});
